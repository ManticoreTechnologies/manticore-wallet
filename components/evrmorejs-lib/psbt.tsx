...
import { Psbt as PsbtBase } from '@/components/evrmorejs-lib/bip174/psbt';
import * as varuint from '@/components/evrmorejs-lib/bip174/converter/varint';
import {
  Bip32Derivation,
  KeyValue,
  PartialSig,
  ...
  TapKeySig,
  TapScriptSig,
} from '@/components/evrmorejs-lib/bip174/interfaces';
import { checkForInput, checkForOutput } from '@/components/evrmorejs-lib/bip174/utils';
import { fromOutputScript, toOutputScript } from '@/components/evrmorejs-lib/Address';
import { cloneBuffer, reverseBuffer } from '@/components/evrmorejs-lib/bufferutils';
import { bitcoin as btcNetwork, Network } from '@/components/evrmorejs-lib/Networks';
import * as payments from '@/components/evrmorejs-lib/Payments';
import { tapleafHash } from '@/components/evrmorejs-lib/Payments/bip341';
import * as bscript from '@/components/evrmorejs-lib/Script';
import { Output, Transaction } from '@/components/evrmorejs-lib/Transaction';
import {
  toXOnly,
  tapScriptFinalizer,
...
export class Psbt {
  ...
  addOutput(outputData: PsbtOutputExtended): this {
    ColorLogger.log(["addOutput", "yellow", "underscore"], ["Adding output", "green", "bright"])
    if (
      arguments.length > 1 ||
      !outputData ||
    ...
    if (typeof address === 'string') {
      ...
      let script = toOutputScript(address, network);
...
type PsbtOutputExtended = PsbtOutputExtendedAddress | PsbtOutputExtendedScript | PsbtOutputExtendedAsset;

interface PsbtOutputExtendedAddress extends PsbtOutput {
  address: string;
  value: number;
}
interface PsbtOutputExtendedAsset extends PsbtOutput {
  address: string;
  assetName: string;
  assetAmount: number;
  value: number;
...
function scriptCheckerFactory(
  ...
  return (
    ...
    ColorLogger.log(["This will tell us about scriptPubKey", "red", "bright"])
...
function prepareFinalScripts(
  ...
  console.log("Preparing final scripts")
...
function getPayment(
  ...
  console.log("Getting payment now")
  switch (scriptType) {
    case 'multisig':
...
function getScriptFromInput(
  ...
  if (isTransferAsset(res.script!)) {
    res.isTransferAsset = true;
  }
  return res;
}
...
function getMeaningfulScript(
  ...
  type: 'p2sh' | 'p2wsh' | 'p2sh-p2wsh' | 'transfer_asset' | 'raw';
} {
  const isP2SH = isP2SHScript(script);
  const isP2SHP2WSH = isP2SH && redeemScript && isP2WSHScript(redeemScript);
  const isP2WSH = isP2WSHScript(script);
  const isTransferAssetType = isTransferAsset(script);
...
function classifyScript(script: Buffer): ScriptType {
  ...
  console.log(isTransferAsset(script))
  if (isTransferAsset(script)) return 'transfer_asset'; // Add this line to handle transfer_asset type
  if (isP2WPKH(script)) return 'witnesspubkeyhash';
  if (isP2PKH(script)) return 'pubkeyhash';
  if (isP2MS(script)) return 'multisig';
  if (isP2PK(script)) return 'pubkey';
  console.log("REturning NONSTANDARD!!")
  return 'nonstandard';
}

// Add this function to detect transfer_asset scripts
function isTransferAsset(script: Buffer): boolean {
  const transferAssetOpcode = bscript.OPS.OP_EVR_ASSET;
  console.log("IS ASSET SCRIPT: "+script.includes(transferAssetOpcode))
  return script.includes(transferAssetOpcode);
}
