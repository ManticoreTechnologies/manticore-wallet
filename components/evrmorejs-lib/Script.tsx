...
export function compile(chunks: Buffer | Stack): Buffer {
  ...
  const bufferSize = chunks.reduce((accum: number, chunk) => {
    ...
    if (singleChunkIsBuffer(chunk)) {
      ...
      console.log(pushdata.encodingLength(chunk.length) + chunk.length)
  ...
  chunks.forEach(chunk => {
    ColorLogger.log(["Current Buffer:\n", "blue", "underscore"], [buffer.toString('hex'), "green", "bright"]);

    // data chunk
    if (singleChunkIsBuffer(chunk)) {
      ColorLogger.log(["    Single Chunk:\n", "blue", "underscore"], [chunk.toString('hex'), "green", "bright"]);

      // adhere to BIP62.3, minimal push policy
      const opcode = asMinimalOP(chunk);
      if (opcode !== undefined) {
        ColorLogger.log(["    Adding opcode:\n", "blue", "underscore"], [`${opcode}`, "red", "underscore"]);

        buffer.writeUInt8(opcode, offset);
        offset += 1;
        return;
      }
      /* Find out where the 03 is coming from  03 657672 */
      console.log("Encoding length for chunk: " + chunk.toString('hex'));
  ...
  ColorLogger.log(["Buffer string below", "magenta", "underscore"]);
